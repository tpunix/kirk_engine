...
static void cccLZRFillBuffer(unsigned int *test_mask, unsigned int *mask, unsigned int *buffer, unsigned char **next_in) {
...
static char cccLZRNextBit(unsigned char *buf_ptr1, int *number, unsigned int *test_mask, unsigned int *mask, unsigned int *buffer, unsigned char **next_in) {
...
static int cccLZRGetNumber(signed char n_bits, unsigned char *buf_ptr, char inc, char *flag, unsigned int *mask, unsigned int *buffer, unsigned char **next_in) {
	...
	while (1) {
		...
		if (!cccLZRNextBit(buf_ptr1, 0, &mask, &mask, &buffer, next_in)) {
			...
			*next_out++ = j;
			...
			if (next_seq < (unsigned char*)out) return -2;
