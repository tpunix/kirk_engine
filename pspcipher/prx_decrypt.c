...
int uprx_decrypt(user_decryptor *pBlock)
{
	...
	*pBlock->newsize = elf_size_comp;
	...
	if (pBlock->type == 3) {
		...
		memcpy(buf2, buf1 + 0xd0, 0x04);
		memset(buf2 + 0x04, 0, 0x58);
		...
		prx_xor_key(buf2+0x14, 0x50, pBlock->xor_key1, pBlock->xor_key2);
	...
	if (pBlock->type < 2 || pBlock->type > 7) {
		...
		prx_xor_key_into(pBlock->prx+0x40, 0x70, buf2+0x2c, buf3+0x20);
		memcpy(pBlock->prx+0xb0, buf2 + 0xb0, 0xa0);
	} else {
		prx_xor_key_large(buf2 + 0x80, 0x40, buf3 + 16);

		if (kirk7(buf2 + 0x6c, 0x40, pBlock->code, pBlock->use_polling) < 0) {
			return -7;
		}

		prx_xor_key_into(pBlock->prx + 0x40, 0x40, buf2 + 108, buf3 + 80);
